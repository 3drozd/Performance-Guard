...
function parseValue(value: string): { num: number; prefix: string; suffix: string; decimals: number; originalFormat: string } {
  if (!value || value === '--:--' || value === '--%' || value === '--MB' || value === '-') {
    return { num: 0, prefix: '', suffix: value, decimals: 0, originalFormat: 'placeholder' };
  }

  // Handle duration format: "5m", "2h 15m", "1h", "45s", "<1m", etc.
  if (value === '<1m') {
    return { num: 0, prefix: '', suffix: value, decimals: 0, originalFormat: 'placeholder' };
  }
  ...
  // Handle regular numbers with prefix/suffix (e.g., "45.2%", "1.2 GB", "500.0 MB", "45%")
...
export function useCountUp(value: string, animationKey: number | undefined, duration: number = 400): string {
