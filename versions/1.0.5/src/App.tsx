...
function AppContent() {
  ...
  useEffect(() => {
    // Allow reload if user auth state changed (login/logout)
    const userChanged = previousUserRef.current?.uid !== user?.uid;

    // If ref says initialized but appDataLoaded is false, state was lost (e.g., after system wake)
    // In this case, allow reloading data
    if (isInitializedRef.current && appDataLoaded && !userChanged) {
    ...
    const loadData = async () => {
      try {
        ...
        // Migracja danych: dodaj kategorię dla starych wpisów bez kategorii
        const migratedWhitelist = finalWhitelist.map(entry => ({
          ...entry,
          category: entry.category || detectCategory(entry.name, entry.exe_path)
        }));

        // Zapisz zmigrowane dane jeśli dodano kategorie
        const needsSave = migratedWhitelist.some((entry, idx) =>
          entry.category && entry.category !== finalWhitelist[idx]?.category
        );

        if (needsSave) {
          await saveAppData(migratedWhitelist, finalSessions, finalNextSessionId);
          if (user && isOnline) {
            await firestoreSync.saveWhitelist(migratedWhitelist);
          }
        }

        setWhitelist(migratedWhitelist);
        nextSessionIdRef.current = finalNextSessionId;

        // Initialize session tracker with saved sessions
        // Clear existing tracker to prevent duplication on re-login
      ...
      if (whitelist.length > 0) {
        ...
        const newSummaries: AppSummary[] = whitelist.map(w => {
          ...
          return {
            ...
            category: w.category, // Kategoria z whitelist
            // Time tracking
  ...
  const handleToggleWhitelist = useCallback((processName: string) => {
    setWhitelist(prev => {
      ...
      const newEntry: WhitelistEntry = {
        ...
        is_tracked: true,
        category: detectCategory(processName, proc?.exe_path), // AUTO-DETEKCJA
      };
  ...
  // Create appIcons Map for DailySummary
  const appIconsMap = useMemo(() => {
    const map = new Map<string, string>();
    appSummaries.forEach(app => {
      if (app.icon) map.set(app.name, app.icon);
    });
    return map;
  }, [appSummaries]);
